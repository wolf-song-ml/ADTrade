...
import com.z.biz.common.conf.Constants
...
object StringUtils {
  ...
  def getFieldFromConcatString(str: String, delimiter: String, field: String): String = {
    try {
      val fields = str.split(delimiter, -1)
      for (concatField <- fields) {
        // searchKeywords=|clickCategoryIds=1,2,3
        val fieldArray = concatField.split("=")
        if (fieldArray.length == 2) {
          val fieldName = fieldArray(0)
          val fieldValue = fieldArray(1)
